      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="relative">
          ...
          {experiences.map((exp, index) => (
            <div key={index} ref={(el) => { itemRefs.current[index] = el; }} className={`relative mb-16 flex flex-col md:flex-row items-start md:items-center w-full ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
              
              {/* DATE PILL */}
              <div className={`md:w-1/2 flex z-10 mb-4 md:mb-0 ${index % 2 === 0 ? 'md:justify-start md:pl-8' : 'md:justify-end md:pr-8'}`}>
                <div className="px-5 py-2 rounded-full text-white font-bold text-sm shadow-lg shadow-black/10" style={{background: 'linear-gradient(to right, var(--theme-from), var(--theme-via))'}}>
                  {exp.date}
                </div>
              </div>
              ...
              <div className="md:w-1/2 w-full px-0 md:px-12">
                <div className="glass-card p-6 md:p-8 rounded-[2rem] border border-white/50 shadow-xl hover:scale-[1.02] hover:shadow-2xl transition-all duration-300">
